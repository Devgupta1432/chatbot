(async function () {
  ...
  widget.innerHTML = `
    <div id="pixora-box" class="hidden">
      <div id="pixora-header" style="background:${config.brandColor}">
        <span class="px-header-circle2"></span>
        ...
        <div id="pixora-header-info">
          ...
          <span class="px-status"><span class="px-status-dot"></span> Online &mdash; replies instantly</span>
        </div>
        <div id="pixora-header-actions">
      ...
      ${config.showBranding ? `<div id="pixora-footer">Powered by <a href="https://pixora.in" target="_blank" rel="noopener noreferrer">Pixora AI</a></div>` : ""}
    </div>
  ...
  toggle.addEventListener("click", () => {
    ...
    const dot = document.getElementById("pixora-notif-dot");
    if (dot && !isHidden) dot.style.display = "none";
    toggle.innerHTML = isHidden
      ? `<svg viewBox="0 0 24 24"><path d="M20 2H4a2 2 0 00-2 2v18l4-4h14a2 2 0 002-2V4a2 2 0 00-2-2z"/></svg><span id="pixora-notif-dot" style="display:none"></span>`
...
function respond(text) {
  ...
  function showContact() {
    ...
    if (config.website) lines.push(`🌐 <a href="${config.website}" target="_blank" rel="noopener noreferrer" style="color:${config.brandColor}">${config.website}</a>`);
  ...
  function addMsg(html, type, quickReplies = []) {
    ...
    // Add timestamp
    const now = new Date();
    el.setAttribute("data-time", now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }));
